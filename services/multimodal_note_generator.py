...
class MultimodalNoteGenerator:
    def __init__(self, logger: Optional[logging.Logger] = None, task_id: Optional[str] = None):
        s=get_settings();key=s.COHERE_API_KEY
        if not key:raise ValueError("COHERE_API_KEY 不能为空")
        self.logger=logger or logging.getLogger(__name__)
        try:
            self.multimodal_service=MultimodalService(
                cohere_api_key=key,ffmpeg_path=s.FFMPEG_PATH,
                ...
                logger=self.logger,task_id=task_id
            )
